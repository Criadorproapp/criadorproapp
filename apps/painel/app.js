...
function connectWS() {
    // Escuta na mesma porta mestre transparente em que a UI foi servida
    ...
    socket.onmessage = (event) => {
        ...
        if (data.type === 'chat-response') {
            ...
            refreshFileTree();
        } else if (data.type === 'jarvis-alert') {
            addMessage('jarvis', data.text);
            createAgentPopup('CRONOGRAMA', data.text);
        }
    };
...
window.toggleView = function(viewType) {
    ...
    if (viewType === 'tree' && tree) tree.style.display = 'block';
    if (viewType === 'graph' && graph) graph.style.display = 'block';
    if (viewType === 'tasks' && tasks) {
...
window.carregarTarefas = async function() {
    try {
        ...
        if (tarefas.length === 0) {
            lista.innerHTML = '<li style="color: #666; text-align: center;">Nenhuma tarefa pendente.</li>';
            return;
        }
        ...
        tarefas.forEach(t => {
            ...
            li.style.cssText = `border: 1px solid #333; margin-bottom: 5px; padding: 8px; display: flex; justify-content: space-between; align-items: center; ${t.concluida ? 'opacity: 0.5; text-decoration: line-through;' : ''}`;
            const dataStr = new Date(t.dataHora).toLocaleString('pt-BR');
            li.innerHTML = `
                <div>
                    <strong style="color: var(--accent);">${t.titulo}</strong><br>
                    <span style="color: #888;">${dataStr}</span>
                </div>
                ${!t.concluida ? `<button onclick="concluirTarefa('${t.id}')" style="background:var(--success); color:#fff; border:none; border-radius:3px; cursor:pointer;">✓</button>` : ''}
            `;
            lista.appendChild(li);
        });
    } catch (e) { console.error("Erro nas tarefas", e); }
};

window.adicionarTarefa = async function() {
    const titulo = document.getElementById('task-titulo').value;
    const dataHora = document.getElementById('task-dataHora').value;
    ...
    document.getElementById('task-dataHora').value = '';
    carregarTarefas();
};

window.concluirTarefa = async function(id) {
    await fetch(`/api/tasks/${id}/concluir`, { method: 'PUT' });
    carregarTarefas();
};
